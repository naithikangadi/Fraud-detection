...
# 1. LOAD & CLEAN (Keeping logic identical to your LightGBM script)
df = pd.read_csv('final_dataset-rk.csv', low_memory=False)

drop_cols = ['Unnamed: 0', 'trans_num', 'unix_time', 'first', 'last', 'street', 'city', 'random_noise_1', 'random_noise_2', 'zip', 'merch_zipcode']
...
X_train, X_test, y_train, y_test, merch_train, merch_test = train_test_split(
    X, y, merchant_col, test_size=0.2, random_state=42, stratify=y
)

# 5. TARGET ENCODE MERCHANT (Crucial to do after split to avoid leakage)
...
# 1. Fill NaNs (Imputation) - Logistic Regression cannot handle NaNs
...
# class_weight='balanced' handles the imbalance similarly to scale_pos_weight
